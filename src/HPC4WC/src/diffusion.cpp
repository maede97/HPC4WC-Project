        ...
        for (block_i = 0; block_i < f.num_i() - BLOCK_SIZE_I + 1; block_i += BLOCK_SIZE_I) {
            ...
            for (block_j = 0; block_j < f.num_j() - BLOCK_SIZE_J + 1; block_j += BLOCK_SIZE_J) {
                for (Field::idx_t i = 0; i < BLOCK_SIZE_I; i++) {
                    for (Field::idx_t j = 0; j < BLOCK_SIZE_J; j++) {
                        tmp(i, j) = STENCIL(block_i + i, block_j + j, k);
                    }
                }

                if (block_j > 0) {
                    // write old col_halo
                    f.setFrom(col_halo, block_i, block_j - f.num_halo(), k);
                }
                ...
                f.setFrom(tmp.block(0, 0, BLOCK_SIZE_I - f.num_halo(), BLOCK_SIZE_J - f.num_halo()), block_i, block_j, k);
            }
            ...
            if (block_j < f.num_j()) {
                ...
                for (Field::idx_t j = block_j; j < f.num_j(); j++) {
                    for (Field::idx_t i = 0; i < BLOCK_SIZE_I; i++) {
                        tmp(i, j - block_j) = STENCIL(i + block_i, j, k);
                    }
                }

                row_halo_current.block(0, block_j, f.num_halo(), num_cols) = tmp.block(BLOCK_SIZE_I - f.num_halo(), 0, f.num_halo(), num_cols);

                f.setFrom(tmp.block(0, 0, BLOCK_SIZE_I - f.num_halo(), num_cols), block_i, block_j, k);
            }

            if (block_i > 0) {
                // now we write tmp2_copy
                f.setFrom(row_halo_old, block_i - f.num_halo(), 0, k);
            }
        ...
        if (block_i < f.num_i()) {
            ...
            for (Field::idx_t i = block_i; i < f.num_i(); i++) {
                for (Field::idx_t j = 0; j < f.num_j(); j++) {
                    tmp_(i - block_i, j) = STENCIL(i, j, k);
                }
            }
            f.setFrom(tmp_, block_i, 0, k);
        }

        // write final tmp2
        f.setFrom(row_halo_current, block_i - f.num_halo(), 0, k);
    }
...
#pragma omp parallel for shared(f) firstprivate(tmp, row_halo, a1, a2, a8, a20) default(none)
    ...
    for (Field::idx_t k = 0; k < f.num_k(); k++) {
        ...
        for (block_i = 0; block_i < f.num_i() - BLOCK_SIZE_I + 1; block_i += BLOCK_SIZE_I) {
            for (Field::idx_t i = 0; i < BLOCK_SIZE_I; i++) {
                for (Field::idx_t j = 0; j < f.num_j(); j++) {
                    tmp(i, j) = STENCIL(block_i + i, j, k);
                }
            }
            // write old row_halo
            if (block_i > 0) {
                f.setFrom(row_halo, block_i - f.num_halo(), 0, k);
            }

            row_halo = tmp.block(BLOCK_SIZE_I - f.num_halo(), 0, f.num_halo(), f.num_j());
            f.setFrom(tmp.block(0, 0, BLOCK_SIZE_I - f.num_halo(), f.num_j()), block_i, 0, k);
        }
        ...
        if (block_i < f.num_i()) {
            ...
            for (Field::idx_t i = block_i; i < f.num_i(); i++) {
                for (Field::idx_t j = 0; j < f.num_j(); j++) {
                    tmp(i - block_i, j) = STENCIL(i, j, k);
                }
            }
            f.setFrom(tmp.block(0, 0, final_rows, f.num_j()), block_i, 0, k);
        }
...
#pragma omp parallel for shared(f) firstprivate(tmp, col_halo, a1, a2, a8, a20) default(none)
    ...
    for (Field::idx_t k = 0; k < f.num_k(); k++) {
        ...
        for (block_j = 0; block_j < f.num_j() - BLOCK_SIZE_J + 1; block_j += BLOCK_SIZE_J) {
            for (Field::idx_t j = 0; j < BLOCK_SIZE_J; j++) {
                for (Field::idx_t i = 0; i < f.num_i(); i++) {
                    tmp(i, j) = STENCIL(i, block_j + j, k);
                }
            }
            // write old col_halo
            if (block_j > 0) {
                f.setFrom(col_halo, 0, block_j - f.num_halo(), k);
            }

            col_halo = tmp.block(0, BLOCK_SIZE_J - f.num_halo(), f.num_i(), f.num_halo());
            f.setFrom(tmp.block(0, 0, f.num_i(), BLOCK_SIZE_J - f.num_halo()), 0, block_j, k);
        }
        ...
        if (block_j < f.num_j()) {
            ...
            for (Field::idx_t j = block_j; j < f.num_j(); j++) {
                for (Field::idx_t i = 0; i < f.num_i(); i++) {
                    tmp(i, j - block_j) = STENCIL(i, j, k);
                }
            }
            f.setFrom(tmp.block(0, 0, f.num_i(), final_cols), 0, block_j, k);
        }
